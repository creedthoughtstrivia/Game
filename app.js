...
const categorySolo= el('#categorySolo');
...
function loadSettings() {
  try {
    const override = JSON.parse(localStorage.getItem('sts_settings') || '{}');
...
// Build a category → sets map to filter question sets
const categoryMap = {};
...
// Populate category dropdown and initial set dropdown
(async function init() {
  // Categories
  APP.CATEGORIES.forEach(cat => {
    ...
    opt.value = cat;
    opt.textContent = cat;
    categorySolo.appendChild(opt);
  });
  categorySolo.addEventListener('change', onCategoryChange);
  // Populate first category sets
  onCategoryChange();
  refreshLB();
})();

function onCategoryChange() {
  const cat = categorySolo.value;
  setSolo.innerHTML = '';
  (categoryMap[cat] || []).forEach(set => {
    const opt = document.createElement('option');
    opt.value = set.id;
    opt.textContent = set.title;
    setSolo.appendChild(opt);
  });
}
...
btnStart.addEventListener('click', async () => {
  ...
  const count = Math.max(5, Math.min(50, parseInt(countSolo.value || '10', 10)));
...
async function finishQuiz() {
  ...
  resultSummary.innerHTML = `<p><b>${state.name}</b> scored <b>${state.score}</b> in ${Math.round(durationMs / 1000)} s.</p>`;
  if (fbReady()) {
...
btnShare.addEventListener('click', async () => {
  const txt = resultSummary.textContent.trim() + ' #ScrantonTriviaShowdown';
  try {
